...
///------------------------------------------------------------------
// Getting all pdfs / ebooks

function getAllEbooks($db){
    $query="SELECT * FROM e_books ORDER BY id DESC";
    $run=mysqli_query($db,$query);
    $data = array();
...
//--------------------Upload EBooks in the form of pdfs--------------------------
...
$ebook_title = mysqli_real_escape_string($db,$_POST['ebook_title']);
$Ebook_category = mysqli_real_escape_string($db,$_POST['Ebook_category']);
$about_ebook = mysqli_real_escape_string($db,$_POST['about_ebook']);
...
$msg = "";
   ...
   // echo "Working..";
	...
	$tempname = $_FILES["uploadfile"]["tmp_name"];
		...
		if (move_uploaded_file($tempname, $folder)) {
			// Get all the submitted data from the form
		    $sql = "INSERT INTO `e_books`( `title`, `pdf_file_name`, `category_id`,`about_ebook`) VALUES('$ebook_title', '$filename','$Ebook_category', '$about_ebook')";
            // Execute query
            ...
            if($run)
            {
                ...
                // header('location:../admin/index.php?all_books');
            }
