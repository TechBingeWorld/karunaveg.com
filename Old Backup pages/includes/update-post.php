...
$pid = mysqli_real_escape_string($db,$_POST['post_id']);
...
$cid= mysqli_real_escape_string($db, $_POST['post_category']);

// echo $cid;

    $query="UPDATE `posts` SET `title`='$ptitle ',`content`='$pcontent',`url`='$url', `about_blog`='$about_blog', `category_id`='$cid' WHERE `id`='$pid'";
   ...
   // echo "Working..";
	...
	$tempname = $_FILES["uploadfile"]["tmp_name"];
		...
		if (move_uploaded_file($tempname, $folder)) {
		    ...
		    $sql = "UPDATE `images` SET `post_id`='$pid',`image`='$filename' WHERE `id`='$image_id'";
            // Execute query
		    mysqli_query($db, $sql);
